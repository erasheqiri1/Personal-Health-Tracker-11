...
const PLAN_KEY = 'mbaj'; // ky file është për planin "Humb Peshë"
...
useEffect(() => {
  ...
  const q = query(
    collection(db, 'meals'),
    where('plan', '==', PLAN_KEY) // pa orderBy
  );
  ...
  return (
    <View style={s.container}>
      ...
      <Text style={s.header}>Ushqimet e planit: Mbaj Peshë</Text>
      ...
      <View style={s.sectionRow}>
        <Pressable
          style={[
            s.sectionChip,
            section === 'menges' && s.sectionChipActive,
          ]}
      ...
      <TextInput
        style={s.input}
        placeholder="Titulli (p.sh. Omeletë 3 vezë + djathë...)"
        value={title}
        onChangeText={setTitle}
      />
      <TextInput
        style={s.input}
        placeholder="Përshkrimi (p.sh. me bukë integrale, vaj ulliri...)"
        value={subtitle}
        onChangeText={setSubtitle}
      />
      <TextInput
        style={s.input}
        placeholder="Kaloritë (p.sh. 520)"
