...
export default function Signup() {
  ...
  const showGenderSelect = () => {
    ...
    if (Platform.OS === 'ios') {
      ActionSheetIOS.showActionSheetWithOptions(
        ...
        (i) => { if (i === 1) setGender('Femër'); if (i === 2) setGender('Mashkull'); }
      );
    } else {
  ...
  const showWeightSelect = () => {
    ...
    if (Platform.OS === 'ios') {
      ActionSheetIOS.showActionSheetWithOptions(
        ...
        (i) => { if (i > 0) setWeight(weights[i - 1]); }
      );
    } else {
  ...
  const showHeightSelect = () => {
    ...
    if (Platform.OS === 'ios') {
      ActionSheetIOS.showActionSheetWithOptions(
        ...
        (i) => { if (i > 0) setHeight(heights[i - 1]); }
      );
    } else {
  ...
  const onSave = async () => {
    const missing = !firstName || !lastName || !username || !password || !agreed;
    if (missing) {
      Alert.alert('Kujdes', 'Ju lutem plotësoni të dhënat.');
      return;
    }
    ...
    try {
      ...
      const idx = users.findIndex((u) => u.emaili === newUser.emaili);
      ...
      router.replace('/(tabs)/dashboard'); // mos shko te profili
    } catch {
      Alert.alert('Gabim', 'Nuk u arrit të ruhen të dhënat e përdoruesit.');
    }
  };

  return (
    <ScrollView contentContainerStyle={s.wrap} keyboardShouldPersistTaps="handled">
      {/* ⬇️ Hiq header-in për këtë ekran */}
